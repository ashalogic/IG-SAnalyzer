...
function CloudWords(props) {
  ...
  props.data.Medias.edges.forEach(x => {
    ...
    if (edges.length > 0 && edges[0].node.text !== "") {
      ...
      caption = caption.replace(new RegExp(ranges.join("|"), "g"), " ");
      ...
      // var captionenstopwords = [];
      // var captionfastopwords = [];
      var captionrealwords = [];
      captionwords.forEach(word => {
        if (word.trim() !== "" && !hasNumber(word.trim()) && word[0] !== "@") {
          if (
      ...
      Captions.push({
        ...
        // enstop: captionenstopwords,
        // fastop: captionfastopwords,
        words: captionrealwords
      });
    }
  });
  var cloudwords = [];
  Captions.forEach(_caption => {
    _caption.words.sort();
    var current = null;
    var cnt = 0;
    for (var i = 0; i < _caption.words.length; i++) {
      if (_caption.words[i] !== current) {
        if (cnt > 0) {
          cloudwords.push({
            text: current,
            value: cnt
  ...
  return (
    ...
    >
      <div className="card-body">
        <ReactWordcloud options={options} words={cloudwords} />
      </div>
    </div>
  );
}
